...
private:
    ...
    void PostRecvBuffers(Connection* conn, int count) {
    ...
    void ProcessRequest(Connection* conn, const void* request_data, size_t request_len,
    ...
    void ProcessGet(Connection* conn, const kv::RequestHeader* header, ConnectionContext* ctx) {
    ...
    void ProcessPut(Connection* conn, const kv::RequestHeader* header, const void* request_data,
    ...
    void SendErrorResponse(Connection* conn, uint64_t key, kv::Status status) {
        Buffer response_buf = buffer_pool_->Allocate();
        if (!response_buf.IsValid()) {
            return;
        }
