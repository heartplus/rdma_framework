...
void KVClient::RunBenchmark(int num_operations, float put_ratio, uint32_t value_size, bool use_sgl,
                            int num_segments) {
...
void KVClient::PostRecvBuffers(size_t count) {
...
void KVClient::SendGet(uint64_t key, uint32_t seq) {
...
void KVClient::SendPut(uint64_t key, uint32_t value_size, uint32_t seq) {
    Buffer send_buf = buffer_pool_->Allocate();
    if (!send_buf.IsValid()) {
        return;
    }
...
void KVClient::SendPutSGL(uint64_t key, uint32_t value_size, int num_segments, uint32_t seq) {
